const coffee = require('../models/index').coffe
const path = require(`path`)
const Op = require(`sequelize`).Op
const fs = require(`fs`)
const upload = require('./image.controller').single('filename')
...
exports.addMenu = async (request, response) => {
    upload(request, response, async error => {
        ...
        // Validasi input dulu sayy
        if (!request.body.name || !request.body.size || !request.body.price) {
            return response.json({ message: "Maaf, keknya ada yang belum kamu isi deh" });
        }

        // Lanjut bro bikin pembuatan menu
        let newMenu = {
...
exports.updateMenu = async (request, response) => {
    upload(request, response, async error => {
        ...
        // ini tuh buat Validasi input yaww
        if (!request.body.name || !request.body.size || !request.body.price) {
            return response.json({ message: "Maaf, keknya ada yang belum kamu isi deh" });
        }
